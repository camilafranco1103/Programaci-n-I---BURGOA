...
for i in range(5):
    message = input(f"Ingrese el palabra numero {1} deseada: ")
    corrimiento = int(input("Ingrese la cant de corrimiento: "))
    message_secret = ""

    for letter in message:
        if letter.isalpha():
            indice = (alphabet.index(letter.upper())+corrimiento)%26
            if letter.isupper():
                message_secret += alphabet[indice].lower()
            else:
                message_secret += alphabet[indice].lower()
        else:
            message_secret += letter
    print(f"El mensaje{i+1} encriptado es {message_secret}")
